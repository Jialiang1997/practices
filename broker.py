...
class Broker:
    ...
    def step(self):
        ...
        for (fd, event) in self.poller.poll(timeout_ms):
                #1 TO SEE IF THERE IS A REQUESTER OR NOT
            if fd == self.requesters:
                self.requests.create_request(self.requesters.recv_multipart())
                #2 IF THE BROKER GOT AN ANSWER THEN DO THE COMMUNICATION BETWEEN SENDER AND RECEIVER
            if fd == self.workers_answer:
                ...
                #3 TO SEE IF ONE WORKER IS IN WORK OR FREE AND THEN ASSIGN 'WORK'
